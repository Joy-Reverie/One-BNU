...
半小时一次的系统周期更新只重绘、不联网，用来跨零点翻页和更新「进行中 / 已结束」标记；此外 `TodayWidgetProvider.scheduleTick`
用一个非精确定时在下一个变化点重绘：没课在上时是下一个上下课时刻，有课在上时是下一个整分（`TodayWidgetModel.nextTick`）。

进行中的那节课下面有一条进度轨道（`layout/widget_track.xml`），一只小猫沿着它向右跑，位置是这节课已过去的比例
（`Row.progress`），所以进行中每分钟要重绘一次。RemoteViews 不能自定义 View 也不能按比例摆放：小猫是 `ProgressBar`
的不确定进度动画 —— 桌面小组件里只有它会自动播放帧动画 —— 帧是 RunCat 365 的 5 帧剪影描成的矢量图（`scripts/gen_cat_frames.py`，
许可见 `third_party/RunCat365/`）；位置按小组件宽度算成像素、用容器的左内边距推过去，已走过的填充在 Android 12 及以上用
`setViewLayoutWidth` 设宽度，更早的系统只留底线与小猫。轨道占掉 `TRACK_DP` 的高度，算能放几行时一并扣掉。
...
本周「今天」那一列上叠一层「现在」指针（`ui/schedule/NowIndicator.kt`）：指针以上到网格顶端蒙一层页面底色，把已经上过的课洗淡；
指针本体是主色细线加左端带光晕的圆点。纵向位置与日程用同一套 `PeriodMapper.position` 换算，因此落点与日程定位一致
（课间并入前一节的行，午休等长空档贴下一节上沿）。时钟 `rememberMinuteClock` 每到整分刷新一次并在回到前台时校准，
以 `State` 的形式往下传、只在绘制 lambda 里读，分钟一跳只重绘指针那一层，七列格子不重组；跨过零点时顺带让 ViewModel 重算「今天」。
...
首页收起时整行整行地展示，上限是列数 × 行数（`ui/home/ServicePinning.kt` 的 `pinnedServiceLimit`）：列数跟宫格一样取
`ScreenInfo.serviceColumns`（手机竖屏四列、很窄的三列，小平板竖屏六列，横屏和大屏八列），行数取 `ScreenInfo.serviceRows`
（手机竖屏即 Compact 三行，其余两行），所以手机上是 12 个、平板横屏是 16 个；转屏、分屏后按新窗口重新算。其余的接在后面、收在宫格下方的箭头里；
展开是同一张宫格往下长，半满那一行的空位原地淡入，不另起一段。宫格在列表底部，展开时顺手把卡片滚进视野。

设置里挑的是「默认展示」哪些入口，存在本机、按校区分开的 `Settings.pinnedServiceEntriesFlow`：`null` 表示没改过，按首页顺序排满收起时的那几行；
改过就只认这个校区有的键、按首页顺序排，超了当前窗口的上限只取前面的（平板横屏上挑满的，转成竖屏照样截断，存的不动）；
写回时存当前实际生效的那份，顺手清掉已经没有的旧键（`ServicePinningTest` 覆盖）。
设置页的预览和首页用同一个列数和上限；开关时布局不动：格子数只跟屏幕走、不跟开关走（空位画成虚线格），
