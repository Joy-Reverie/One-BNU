...
校园服务里的北京数字京师入口固定加载官方电脑端首页，并把 WebView UA 设置为桌面浏览器；后台已有门户 OAuth 会话时同步
accessToken，未预热成功时仍直接保留官方页面入口，不让应用侧认证回退阻断页面。若服务端仍返回跨设备引导页，应用只设置官方页面
要求的本地访问偏好，再回到电脑端首页，不向页面注入账号或密码。
