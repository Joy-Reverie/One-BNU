...
object GpaCalculator {
    ...
    fun summarize(grades: List<Grade>, scale: GpaScale): GpaSummary {
        var pointSum = 0.0
    ...
    /** 按学期分组统计，最近的学期排在前面。 */
    fun byTerm(grades: List<Grade>, scale: GpaScale): List<Pair<String, GpaSummary>> =
        grades.groupBy { it.termLabel }
            .map { (label, list) -> label to summarize(list, scale) }
